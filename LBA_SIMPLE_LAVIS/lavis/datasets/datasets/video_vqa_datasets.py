...
class DramaQAIncludingSubQAEvalDataset(VideoQADataset, __DisplMixin):
    def __init__(self, vis_processor, text_processor, vis_root, ann_paths):
        super().__init__(vis_processor, text_processor, vis_root, ann_paths)
        
        self.prompts = json.load(open('/home/ywjang/LBA_2024/LBA-SNU/prompts.json', "r"))
    ...
    def __getitem__(self, index):
        ...
        prompts = self.prompts["Reasoner"]
        question = ""
        question += prompts["init_prompt"]
        for sub_question, sub_answer in zip(ann["sub_questions"], ann["sub_answers"]):
            question += prompts["pair_prompt"].format(sub_question, sub_answer)
        question += prompts["final_prompt"]
...
class DramaQAIncludingSubQAEvalDataset(VideoQADataset, __DisplMixin):
    def __init__(self, vis_processor, text_processor, vis_root, ann_paths):
        ...
        print('yomyomkos ann_paths:', ann_paths)
        # ann_paths: len=4, [DramaQA QA path, generated SubQ path, generated SubA path, prompt path]
        self.mainQAs = json.load(open(ann_paths[0], "r"))
        subQs = json.load(open(ann_paths[1], "r"))
        subAs = json.load(open(ann_paths[2], "r"))
        self.prompts = json.load(open(ann_paths[3], "r"))
        ...
        for subQ in subQs:
            # print('subQ:', subQ)
            self.subQs[subQ["qid"]] = subQ["sub_questions"]
        for subA in subAs:
            # print('subA:', subA)
            self.subAs[subA["qid"]] = subA["sub_answers"]

    def __getitem__(self, index):
        mainQA = self.mainQAs[index]
        image_path_list = []
        
        qid = mainQA["qid"]
        vname = mainQA["vid"] # AnotherMissOh17_001_0000
        if vname.endswith("0000"): # scene
            vpath = os.path.join(self.vis_root, vname[:-5].replace('_', '/'))
            for shot in glob.glob(vpath + "/*"):
                for frm in glob.glob(shot + "/*"):
        ...
        question = ""
        question += prompts["init_prompt"]
        for sub_Q, sub_A in zip(self.subQs[qid], self.subAs[qid]):
            question += prompts["pair_prompt"].format(sub_Q, sub_A)
        question += prompts["final_prompt"]

        return {
            "image": frms,
            "text_input": question,
            "answer": mainQA["correct_idx"],
            "answer_list": mainQA["answers"],
            "gt_ans": mainQA["answers"][mainQA["correct_idx"]],
            "question_id": mainQA["qid"],
            "instance_id": str(index),
            # "instance_id": mainQA["instance_id"],
        }
        ...
        #     "main_question": mainQA["que"],
        #     "main_answer_idx": mainQA["correct_idx"],
        #     "gt_main_answer": mainQA["answers"][mainQA["correct_idx"]],

        #     "sub_q_list": self.SubQs[index],
        #     "sub_a_list": self.SubAs[index],
        # }
"""
