...
@registry.register_task("vqa_sub_answer")
class SubAnswerTask(VQATask):
    def valid_step(self, model, samples):
        ...
        for pred_answer, ques_id, gt_answer in zip(pred_answers, question_id, gt_answers):
            ques_id = int(ques_id.item())
            # gt_answer = int(gt_answer.item())
            pred_qa_pairs.append({"question_id": ques_id, "pred_ans": pred_answer, "gt_ans": gt_answer})
            pred_qa_pairs_for_save.append({"question_id": ques_id, "pred_ans": pred_answer, "gt_ans": int(gt_answer.item())})
            print('ques_id:', type(ques_id), ques_id)
            print('pred_answer:', type(pred_answer), pred_answer)
            print('gt_answer:', type(gt_answer), gt_answer)

        gt_answers = samples["gt_ans"]
        question_id = samples["question_id"]
        for ques_id, answer, gt_answer  in zip(question_id, answers, gt_answers):
            ques_id = int(ques_id.item())
            pred_qa_pairs.append({"question_id": ques_id, "answer": answer, "gt_ans": gt_answer})

        return pred_qa_pairs

# @registry.register_task("dramaqa_including_subqa_eval_task")
# class DramaQAIncludingSubQAEvalTask(VQATask):
#     def valid_step(self, model, samples):
#         print_sample(samples)
#         answers = model.predict_answers(
#             samples=samples,
...
class DramaQAEvalTask(VQATask):
    # valid_step()에서 samples을 입력 받아 [{ques_id, pred_answer, gt_answer}] list of dict를 반환함
    def __init__(
            self,
            num_beams,
            max_len,
    ...
    def valid_step(self, model, samples):
        ...
        print_sample(samples)
        """
        Evaluation  [ 260/3889]  eta: 1:30:54    time: 1.5733  data: 0.0114  max mem: 29859
        print sample...
        key: image,   value: torch.Size([1, 3, 224, 224])
        key: text_input,      value: ["How did Jinsang feel when Sukyung said that the Brazilian guy was not the baby's father?"]
        key: answer,  value: torch.Size([1])
        key: answer_list,     value: [['Jinsang was sad that all the work went for nothing and had to start it again.'], ['Jinsang was happy to meet the Brazilian guy at the cafe after work.'], ['Jinsang was mad because Dokyung was always late to the meeting.'], ['Jinsang was shocked to hear the fact because Jinsang thought the Brazilian guy was the father.'], ['Jinsang was excited because Jinsang will be on vacation tomorrow.']]
        key: gt_ans,  value: ['Jinsang was shocked to hear the fact because Jinsang thought the Brazilian guy was the father.']
        key: question_id,     value: torch.Size([1])
        key: instance_id,     value: ['261']
        answers: ['Surprised']
        samples["answer_list"]: [['Jinsang was sad that all the work went for nothing and had to start it again.'], ['Jinsang was happy to meet the Brazilian guy at the cafe after work.'], ['Jinsang was mad because Dokyung was always late to the meeting.'], ['Jinsang was shocked to hear the fact because Jinsang thought the Brazilian guy was the father.'], ['Jinsang was excited because Jinsang will be on vacation tomorrow.']]
        answer: Surprised
        similarity_list: [tensor(0.1815, device='cuda:0'), tensor(0.1510, device='cuda:0'), tensor(0.1041, device='cuda:0'), tensor(0.1999, device='cuda:0'), tensor(0.1761, device='cuda:0')]
        pred_answers: 3          Jinsang was shocked to hear the fact because Jinsang thought the Brazilian guy was the father.
        """
        # TODO: DramaQA: edit this function to return the predicted answers
        answers = model.predict_answers(
            samples=samples,
        ...
        return pred_qa_pairs, pred_qa_pairs_for_save
